...
struct PopularPicksView<ProdViewModel>: View where ProdViewModel: ProductViewModelProtocol {
    let products: [ProductEntity] // Uses `ProductEntity`
    @ObservedObject var productViewModel: ProdViewModel
...
extension PopularPicksView {

    // Section Header
    @ViewBuilder
    private var sectionHeader: some View {
        Text(AppConstants.UI.homePopularPicksTitle)
            .font(.title3)
            .fontWeight(.bold)
            .foregroundColor(.purple)
            .padding(.horizontal)
            .padding(.top, 8)
            .accessibilityIdentifier("Popular_Picks")
    }
    ...
    private var cardBackground: some View {
        LinearGradient(
            gradient: Gradient(colors: [Color.white, Color.purple.opacity(0.05)]),
            startPoint: .top,
            endPoint: .bottom
        )
    }
